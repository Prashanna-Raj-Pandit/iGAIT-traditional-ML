...
# 1. Load & Prepare Data
# ── CHANGE: Keep Subject IDs in a separate vector before removing them ──────
df_raw <- read.csv("gait_features_rich.csv", stringsAsFactors = FALSE)
subject_ids <- df_raw$Subject   # <-- save IDs here before dropping
...
# 6. Train / Test Split
# ── CHANGE: Track which row indices go to test set ──────────────────────────
split_idx  <- createDataPartition(df_sel$Class, p = 0.70, list = FALSE)
train_raw  <- df_sel[ split_idx, ]
test_raw   <- df_sel[-split_idx, ]

# ── CHANGE: Save the Subject IDs for the test set rows ──────────────────────
test_subject_ids <- subject_ids[-split_idx]   # IDs matching test rows
...
# 9. Evaluation Helper
# ── CHANGE: Added subject_ids parameter to save predictions per subject ──────
eval_model <- function(model, test_data, name, subj_ids = NULL) {
  ...
  # ── CHANGE: Save subject-level predictions if IDs are provided ─────────────
  if (!is.null(subj_ids)) {
    pred_df <- data.frame(
      Subject       = subj_ids,
      True_Label    = as.character(test_data$Class),
      Predicted     = as.character(preds),
      Prob_ASD      = round(probs, 4),
      Correct       = as.character(preds) == as.character(test_data$Class),
      stringsAsFactors = FALSE
    )
    out_file <- paste0("predictions_", gsub("[^A-Za-z0-9]", "_", name), ".csv")
    write.csv(pred_df, out_file, row.names = FALSE)
    cat(sprintf("  Subject-level predictions saved to: %s\n", out_file))
  }
...
# ── CHANGE: Pass test_subject_ids to save predictions ───────────────────────
results_list[["LR"]] <- eval_model(lr_model, test_scaled,
                                   "Logistic Regression (Elastic Net)",
                                   subj_ids = test_subject_ids)
...
cat("  predictions_Logistic_Regression__Elastic_Net_.csv\n")
cat("  predictions_Random_Forest.csv\n")
cat("  predictions_SVM__RBF_Kernel_.csv\n")
if (exists("xgb_model") && !is.null(xgb_model))
  cat("  predictions_XGBoost.csv\n")